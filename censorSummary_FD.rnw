...
demo.path <- "/scratch2/weie/MARCER/"
...
make.plot <- function(do.age) {   # do.age <- "MA"
  sub.ids <-   demo$sub.id[demo$...3==do.age & demo$Scans==1]
  ...
  for (sid in 1:length(sub.ids)) {    # for (sid in 1:5) {    # sid <- 3; 
    ...
    mtext(side=3, text=paste0(sub.ids[sid], " (MARCER,",do.age,")"), line=0.1, cex=0.6); 
...
\section*{Middle-Age Adults}
<<code1, cache=TRUE, dev='pdf', echo=FALSE, fig.height=2.3, fig.width=7.75, fig.align='center'>>= 
...
out.tbl <- make.plot("MA");

@

\newpage
<<code1a, echo=FALSE, cache=TRUE, size='small'>>=
options(width=100);   # print more columns

print("MARCER Middle-Age Adults proportions", quote=FALSE);
print(out.tbl);
...
\newpage
\section*{MARCER Older Adults}
<<code2, cache=TRUE, dev='pdf', echo=FALSE, fig.height=2.3, fig.width=7.75, fig.align='center'>>= 
layout(matrix(1:3, c(1,3)));
par(mar=c(1.75, 1.75, 1.5, 0.75), mgp=c(1.1, 0.2, 0), tcl=-0.3);
# mar: c(bottom, left, top, right) gives the number of lines of margin to be specified on the four sides of the plot. Default is c(5, 4, 4, 2) + 0.1.

out.tbl <- make.plot("OA");

@

\newpage
<<code2a, echo=FALSE, cache=TRUE, size='small'>>=
options(width=100);   # print more columns

print("MARCER Older Adults proportions", quote=FALSE);
print(out.tbl);
